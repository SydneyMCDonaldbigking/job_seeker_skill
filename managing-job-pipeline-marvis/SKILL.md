...
description: Manage a local job-application pipeline stored as Marvis-compatible Markdown tasks. Use when Codex needs to check job IDs or URLs for duplicates, record discovered or skipped roles, move an application through applying/submitted/interview/closed states, attach resume or confirmation details, migrate legacy job_application_profile.json applications, or validate the local application history before screening or applying.
---
...
3. Record a promising unhandled job with `add`; record a deliberate rejection with `add --status skipped`.
...
python scripts/job_pipeline.py --vault <vault> add --record-json <path>
